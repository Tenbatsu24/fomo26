...
from fomo26.models import vitv2_a_2d_tiny, vitv2_a_3d_tiny
...
class TestViTv2Adaption2D:
    def test_classification(self):
        model = vitv2_a_2d_tiny(med_in_channels=1, task="classification", classes=2).to(DEVICE)
        x = torch.randn(2, 1, 28, 28, 8, device=DEVICE)
        out = model(x)
        assert out.shape == (2, 2)

    def test_regression(self):
        model = vitv2_a_2d_tiny(med_in_channels=1, task="regression", classes=1).to(DEVICE)
        x = torch.randn(2, 1, 28, 28, 8, device=DEVICE)
        out = model(x)
        assert out.shape == (2, 1)

    def test_segmentation(self):
        model = vitv2_a_2d_tiny(med_in_channels=1, task="segmentation", classes=3).to(DEVICE)
        x = torch.randn(2, 1, 28, 28, 8, device=DEVICE)
        out = model(x)
    ...
    def test_task_token_beginning(self):
        model = vitv2_a_2d_tiny(
            med_in_channels=1, task="classification", classes=2,
            task_token=True, task_token_insertion="beginning",
        ).to(DEVICE)
        x = torch.randn(2, 1, 28, 28, 8, device=DEVICE)
        out = model(x)
        assert out.shape == (2, 2)
    ...
    def test_task_token_middle(self):
        model = vitv2_a_2d_tiny(
            med_in_channels=1, task="classification", classes=2,
            task_token=True, task_token_insertion="middle", task_token_block=6,
        ).to(DEVICE)
        x = torch.randn(2, 1, 28, 28, 8, device=DEVICE)
        out = model(x)
        assert out.shape == (2, 2)
