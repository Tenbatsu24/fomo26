...
if __name__ == "__main__":
    ...
    for c in identifiers:
        ...
        print(c)
        tr_file = join(
            "$nnUNet_raw", target_dataset_name_ribfrac, "imagesTr", c + "_0000.nii.gz"
        )
        ts_file = join(
            "$nnUNet_raw", target_dataset_name_ribfrac, "imagesTs", c + "_0000.nii.gz"
        )
        ...
        dataset[c] = {"images": [img_file], "label": join("labelsTr", c + ".nii.gz")}

    generate_dataset_json(
        join(nnUNet_raw, target_dataset_name),
        channel_names={0: "CT"},
        labels={"background": 0, **{"rib%02.0d" % i: i for i in range(1, 25)}},
        num_training_cases=n_tr,
        file_ending=".nii.gz",
        dataset_name=target_dataset_name,
