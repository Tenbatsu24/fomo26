...
if __name__ == "__main__":
    ...
    models = dumb_trainer_config_plans_to_trained_models_dict(
        ...
        ["2d", "3d_lowres", "3d_cascade_fullres", "3d_fullres"],
        ["nnUNetPlans"],
    )
    ...
    for im in ret["best_model_or_ensemble"]["selected_model_or_models"]:
        ...
        model_folder = get_output_folder(
            d, im["trainer"], im["plans_identifier"], im["configuration"]
        )
        ...
        predictor.predict_from_files(
            source_dir, output_dir, has_ensemble, overwrite=True
        )
    ...
    if has_ensemble:
        ensemble_folders(
            output_folders,
            join(target_dir_base, "ensemble_predictions"),
    ...
    pp_fns, pp_fn_kwargs = load_pickle(
        ret["best_model_or_ensemble"]["postprocessing_file"]
    )
    apply_postprocessing_to_folder(
        folder_for_pp,
