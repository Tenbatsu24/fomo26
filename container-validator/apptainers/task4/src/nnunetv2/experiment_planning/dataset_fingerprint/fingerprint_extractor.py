...
class DatasetFingerprintExtractor(object):
    def __init__(
        ...
        self.dataset = get_filenames_of_train_images_and_targets(
            self.input_folder, self.dataset_json
        )
    ...
    def collect_foreground_intensities(
        ...
        for i in range(len(images)):
            ...
            mean, median, mini, maxi, percentile_99_5, percentile_00_5 = (
                np.nan,
                np.nan,
                np.nan,
                np.nan,
    ...
    def run(self, overwrite_existing: bool = False) -> dict:
        ...
        if not isfile(properties_file) or overwrite_existing:
            ...
            foreground_intensities_per_channel = np.array(
                foreground_intensities_per_channel
            )
