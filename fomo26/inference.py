...
def sliding_window_predict(
    ...
    if is_2d:
        ...
        spatial_vol = (H, W)
        spatial_patch = (ph, pw)
        spatial_overlap = (overlap[0], overlap[1]) if len(overlap) >= 2 else (overlap[0], overlap[0])
    else:
        # 3-D model: slide over D, H, W
        ph, pw, pd = patch_size[0], patch_size[1], patch_size[2]
        spatial_vol = (D, H, W)
        spatial_patch = (ph, pw, pd)
        spatial_overlap = (overlap[0], overlap[1], overlap[2]) if len(overlap) >= 3 else (overlap[0], overlap[1], overlap[2])
    ...
    for batch_start in range(0, len(starts), batch_size):
        ...
        for si, ei in zip(batch_indices, batch_ends):
            ...
            if is_2d:
                pd_act = D  # full depth
                padded = torch.zeros(
                    cur_batch_size, C_in, ph, pw, pd_act,
                ...
                pd_act = ei[2] - si[2]
                padded = torch.zeros(
                    cur_batch_size, C_in, pd_act, ph, pw,
                    device=device, dtype=volume.dtype,
                )
        ...
        if C_out != output_channels:
            output_acc = torch.zeros(B, C_out, H, W, D, device=device, dtype=torch.float32)
            weight_acc = torch.zeros(B, 1, H, W, D, device=device, dtype=torch.float32)
