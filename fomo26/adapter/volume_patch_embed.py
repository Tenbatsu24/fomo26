...
if __name__ == "__main__":
    ...
    print("--- 1. Initializing PatchEmbed3D (HWD Format) ---")
    ...
    print(f"Input Volume Shape (H, W, D): {img_size}")
    print(f"Patch Size (h, w, d): {patch_size}")
    print(
        f"Expected Patch Grid Size (H_grid, W_grid, D_grid): {patch_embed.patches_resolution}"
    )
    print(f"Expected Number of Patches: {patch_embed.num_patches}")
    print(f"Calculated GFLOPs: {patch_embed.flops() / 1e9:.6f} GFLOPs")
    print("-" * 50)

    # 3. Create a mock tensor in the HWD format: (B, C, H, W, D)
    x = torch.randn(batch_size, in_chans, *img_size)
    print(f"Mock Input Tensor Shape: {x.shape}")

    # 4. Test Flattened Output: (B, N, E)
    out_flat = patch_embed(x)
    print("\n--- 2. Testing Flattened Output ---")
    ...
    print("v Flattened Output Shape is correct!")
    ...
    print("\n--- 3. Testing Unflattened Output (HWD Layout) ---")
    print(f"Output Shape: {out_unflat.shape}")
    ...
    print("v Unflattened HWD Output Shape is correct!")
    print("-" * 50)
