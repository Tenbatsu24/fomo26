...
from med_adapt.adapter import InputChannelAdapter, AttentionPooling, TaskTokens
    ...
    def prepare_tokens_with_masks(self, x, masks=None):
        ...
        if masks is not None:
            ...
            if masks.shape[0] == B_vol:
                masks_exp = (
                    masks.unsqueeze(1).expand(-1, c, -1, -1).reshape(B_orig, -1, 1)
                )
                x = torch.where(
                    masks_exp.bool(), self.mask_token.to(x.dtype).unsqueeze(0), x
                )
