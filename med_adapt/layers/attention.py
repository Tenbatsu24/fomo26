...
class CrossAttentionBlock(nn.Module):
    """
    Cross-attention block: learnable query tokens attend over a sequence of
    key/value tokens (e.g. patch tokens from all 3D slices flattened together).

    queries:      (B, Q, E)
    keys_values:  (B, N, E)
    returns:      (B, Q, E)

    `attn_bias` is accepted for signature consistency with self-attention
    blocks but is not used (keep as None).
    """
    ...
    def forward(self, queries: Tensor, keys_values: Tensor, attn_bias=None) -> Tensor:
        ...
        k = (
            self.k_proj(keys_values)
            .reshape(B, N, self.num_heads, self.head_dim)
            .transpose(1, 2)
        )
        v = (
            self.v_proj(keys_values)
            .reshape(B, N, self.num_heads, self.head_dim)
            .transpose(1, 2)
        )
