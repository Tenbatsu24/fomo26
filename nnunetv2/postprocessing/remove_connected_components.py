...
def determine_postprocessing(
    ...
    if plans_file_or_dict is None:
        ...
        if not isfile(expected_plans_file):
            raise RuntimeError(
                f"Expected plans file missing: {expected_plans_file}. The plans files should have been "
    ...
    if not all([i in predicted_files for i in ref_files]):
        print(
            "WARNING: Not all files in folder_ref were found in folder_predictions. Determining postprocessing "
