...
def convert(source_folder, target_dataset_name):
    ...
    shutil.copytree(
        ...
        join(nnUNet_raw, target_dataset_name, "imagesTr"),
    )
    shutil.copytree(
        join(source_folder, "labelsTr"),
        join(nnUNet_raw, target_dataset_name, "labelsTr"),
    )
    if isdir(join(source_folder, "imagesTs")):
        shutil.copytree(
            join(source_folder, "imagesTs"),
            join(nnUNet_raw, target_dataset_name, "imagesTs"),
        )
    if isdir(join(source_folder, "labelsTs")):
        shutil.copytree(
            join(source_folder, "labelsTs"),
            join(nnUNet_raw, target_dataset_name, "labelsTs"),
        )
    if isdir(join(source_folder, "imagesVal")):
        shutil.copytree(
            join(source_folder, "imagesVal"),
            join(nnUNet_raw, target_dataset_name, "imagesVal"),
        )
    if isdir(join(source_folder, "labelsVal")):
        shutil.copytree(
            join(source_folder, "labelsVal"),
            join(nnUNet_raw, target_dataset_name, "labelsVal"),
        )
    shutil.copy(
        join(source_folder, "dataset.json"), join(nnUNet_raw, target_dataset_name)
    )
