...
def copy_files(
    ...
    for patient in patients_train:
        ...
        shutil.copy(
            train_file, train_dir / f"{train_file.stem.split('.')[0]}_0000.nii.gz"
        )
...
def convert_emidec(src_data_dir: str, src_test_dir: str, dataset_id=27):
    out_dir, train_dir, labels_dir, test_dir = make_out_dirs(
        dataset_id=dataset_id, task_name="EMIDEC"
    )
    num_training_cases = copy_files(
        Path(src_data_dir), Path(src_test_dir), train_dir, labels_dir, test_dir
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i", "--input_dir", type=str, help="The EMIDEC dataset directory."
    )
    parser.add_argument(
        "-t", "--test_dir", type=str, help="The EMIDEC test set directory."
    )
    parser.add_argument(
        "-d",
        "--dataset_id",
        ...
        type=int,
        default=115,
        help="nnU-Net Dataset ID, default: 115",
    )
