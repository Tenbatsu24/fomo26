...
if __name__ == "__main__":
    ...
    for c in tqdm(case_ids_hgg):
        shutil.copy(
            join(brats_data_dir, "HGG", c, c + "_t1.nii"),
            join(imagestr, c + "_0000.nii"),
        )
        shutil.copy(
            join(brats_data_dir, "HGG", c, c + "_t1ce.nii"),
            join(imagestr, c + "_0001.nii"),
        )
        shutil.copy(
            join(brats_data_dir, "HGG", c, c + "_t2.nii"),
            join(imagestr, c + "_0002.nii"),
        )
        shutil.copy(
            join(brats_data_dir, "HGG", c, c + "_flair.nii"),
            join(imagestr, c + "_0003.nii"),
        )

        copy_BraTS_segmentation_and_convert_labels_to_nnUNet(
            join(brats_data_dir, "HGG", c, c + "_seg.nii"), join(labelstr, c + ".nii")
        )
    print("copying lggs")
    for c in tqdm(case_ids_lgg):
        shutil.copy(
            join(brats_data_dir, "LGG", c, c + "_t1.nii"),
            join(imagestr, c + "_0000.nii"),
        )
        shutil.copy(
            join(brats_data_dir, "LGG", c, c + "_t1ce.nii"),
            join(imagestr, c + "_0001.nii"),
        )
        shutil.copy(
            join(brats_data_dir, "LGG", c, c + "_t2.nii"),
            join(imagestr, c + "_0002.nii"),
        )
        shutil.copy(
            join(brats_data_dir, "LGG", c, c + "_flair.nii"),
            join(imagestr, c + "_0003.nii"),
        )

        copy_BraTS_segmentation_and_convert_labels_to_nnUNet(
            join(brats_data_dir, "LGG", c, c + "_seg.nii"), join(labelstr, c + ".nii")
        )
