...
def convert_autopet(
    autopet_base_dir: str = "/media/isensee/My Book1/AutoPET/nifti/FDG-PET-CT-Lesions",
    ...
    for pat in patients:
        ...
        for pa in patient_acquisitions:
            ...
            if not isfile(join(imagestr, f"{identifier}_0000.nii.gz")):
                shutil.copy(
                    join(autopet_base_dir, pat, pa, "CTres.nii.gz"),
                    join(imagestr, f"{identifier}_0000.nii.gz"),
                )
            if not isfile(join(imagestr, f"{identifier}_0001.nii.gz")):
                shutil.copy(
                    join(autopet_base_dir, pat, pa, "SUV.nii.gz"),
                    join(imagestr, f"{identifier}_0001.nii.gz"),
                )
            if not isfile(join(imagestr, f"{identifier}.nii.gz")):
                shutil.copy(
                    join(autopet_base_dir, pat, pa, "SEG.nii.gz"),
                    join(labelstr, f"{identifier}.nii.gz"),
                )
        ...
        splits.append(
            {
                "train": [
                    ...
                    if not any([i.startswith(v) for v in val_patients])
                ],
                "val": [
                    i
                    for i in identifiers
                    if any([i.startswith(v) for v in val_patients])
                ],
            }
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=221,
        help="nnU-Net Dataset ID, default: 221",
    )
