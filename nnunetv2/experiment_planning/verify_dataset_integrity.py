...
def check_cases(
    ...
    if np.any(np.isnan(segmentation)):
        print(
            f"Segmentation contains NaN pixel values. You need to fix that.\nSegmentation:\n{label_file}"
        )
        ret = False
    ...
    if "nibabel_stuff" in properties_image.keys():
        ...
        if not np.allclose(affine_image, affine_seg):
            print(
                "Warning: Affine mismatch between segmentation and corresponding images. "
                "\nAffine image: %s \nAffine seg: %s\n"
                "Image files: %s. \nSeg file: %s.\n"
...
def verify_dataset_integrity(folder: str, num_processes: int = 8) -> None:
    ...
    """
    assert isfile(
        join(folder, "dataset.json")
    ), f"There needs to be a dataset.json file in folder, folder={folder}"
    dataset_json = load_json(join(folder, "dataset.json"))

    if "dataset" not in dataset_json.keys():
        assert isdir(
            join(folder, "imagesTr")
        ), f"There needs to be a imagesTr subfolder in folder, folder={folder}"
        assert isdir(
            join(folder, "labelsTr")
        ), f"There needs to be a labelsTr subfolder in folder, folder={folder}"
    ...
    assert len(dataset) == expected_num_training, (
        ...
        "(%d). Found %d instead.\nExamples: %s"
        % (expected_num_training, len(dataset), list(dataset.keys())[:5])
    )
