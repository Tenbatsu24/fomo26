...
def preprocess_like_nnssl(
    ...
    if os.path.isdir(preprocessed_folder) and not overwrite_pp:
        ...
        print(
            "Skipping preprocessing. Pass --overwrite_pp (or delete the folder) to re-preprocess."
        )
    else:
        ...
        preprocessor.run(
            dataset_id, "3d_fullres", plans_name, num_processes=num_processes
        )
...
def plan_like_dynamic(
    ...
    print(
        f"Saved dynamic adaptation plan to: {join(nnUNet_preprocessed, dataset_name, plans_name + '.json')}"
    )
    ...
    print(
        f"  nnUNetv2_train_pretrained {dataset_id} 3d_fullres FOLD -p {plans_name} -tr DynamicPretrainedTrainer"
    )
...
def plan_like_dynamic_entrypoint():
    ...
    pretrained_checkpoint_path = maybe_download_pretrained_weights(
