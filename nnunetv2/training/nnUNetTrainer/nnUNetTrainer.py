...
class nnUNetTrainer(object):
    def __init__(
        ...
        self.output_folder_base = (
            ...
            if nnUNet_results.is_set()
            else None
        )
        self.output_folder = (
            join(self.output_folder_base, f"fold_{fold}")
        ...
        self.preprocessed_dataset_folder = (
            ...
            if self.preprocessed_dataset_folder_base is not None
            else None
        )
                ...
                if k in ["dataloader_train", "dataloader_val"]:
                    ...
                    if hasattr(dl, "generator"):
                        ...
                        if hasattr(dl.generator, "transforms"):
                            try:
                                dct[k + ".generator.transforms"] = str(
                                    dl.generator.transforms
                                )
                            except Exception as e:
                                dct[k + ".generator.transforms"] = (
                                    f"Could not stringify generator.transforms: {type(e).__name__}: {e}"
                                )
    ...
    def on_epoch_end(self):
        ...
        if (
            ...
            or self.logger.get_value("ema_fg_dice", step=-1) > self._best_ema
        ):
        ...
        if self.local_rank == 0:
            ...
            for label in metrics["mean"]:
                self.logger.log_summary(
                    f"final_val/class_{label}_dice", metrics["mean"][label]["Dice"]
                )
            self.logger.log_summary(
                "final_val/foreground_dice", metrics["foreground_mean"]["Dice"]
            )
    ...
    def run_training(self):
        ...
        for _ in tqdm(range(self.current_epoch, self.num_epochs)):
            self.on_epoch_start()
