...
class PretrainedTrainer(nnUNetTrainer):
    def __init__(
        ...
        self.use_pretrained_weights = (
            self.adaptation_info.get("checkpoint_path") is not None
        )
        ...
        try:
            ...
            if len(cits) > 0:
                ...
                for cit in sorted(cits, key=lambda x: x.get("type", "")):
                    all_strings.append(
                        f"{cit.get('type', '?')} used '{cit.get('name', '?')}'. Associated paper(s):"
                    )
                    papers = (
                        cit.get("apa_citations") or cit.get("bibtex_citations") or []
                    )
                    for c in papers:
    ...
    def load_pretrained_weights(
        ...
        pre_train_statedict: dict[str, torch.Tensor] = ckp[
            "network_weights"
        ]  # Get pre-trained state dict
        ...
        if stem_in_encoder:
            ...
            if need_to_adapt_lpe:
                if lpe_in_encoder:
                    ...
                    new_encoder_weights["pos_embed"].to(
