...
class DynamicPretrainedTrainer(PretrainedTrainer):
        ...
        if not self.was_initialized:
            ...
            if self.use_pretrained_weights:
                ...
                self.network, self.pt_weight_in_ch_mismatch = (
                    self.load_pretrained_weights_dynamic(
                        self.network,
                ...
                self.print_to_log_file(
                    "Loaded Network (with dynamic adaptation) from {}".format(
                        self.adaptation_info["checkpoint_path"]
                    )
                )
            else:
                self.print_to_log_file(
                    "You are using a Trainer for fine-tuning but without loading weights"
                )
    ...
    def load_pretrained_weights_dynamic(
        ...
        pt_arch_kwargs = ckp.get("nnssl_adaptation_plan", {}).get(
            "architecture_kwargs", {}
        )
        ...
        if "architecture_kwargs" not in ckp.get("nnssl_adaptation_plan", {}):
            ...
            encoder_keys = [
                k for k in pre_train_statedict.keys() if k.startswith(pt_key_to_encoder)
            ]
            stage_indices = set()
            ...
            print(
                f"[Dynamic Adaptation] Inferred pretrained encoder has {pt_n_stages} stages"
            )
        ...
        if stem_in_encoder:
            ...
            new_encoder_weights, adaptation_log = (
                adapt_encoder_weights_for_architecture(
            ...
            if need_to_adapt_lpe:
                if lpe_in_encoder and "pos_embed" in new_encoder_weights:
                    handle_pos_embed_resize(
                        ...
                        pt_input_patchsize,
                        new_encoder_weights.get(
                            "down_projection.proj.weight", torch.zeros(1, 1, 1, 1, 1)
                        ).shape[2:],
                    )
                    new_encoder_weights["pos_embed"] = new_encoder_weights[
                        "pos_embed"
                    ].to(next(network.parameters()).device)

            if "cls_token" in new_encoder_weights:
                skip_strings_in_pretrained = ["cls_token"]
                new_encoder_weights, _ = filter_state_dict(
            ...
            for key in list(new_stem_weights.keys()):
                ...
                if (
                    "conv.weight" in key or (key.endswith(".weight") and "conv" in key)
                ) and len(pt_weight.shape) >= 4:
                    pt_k = list(pt_weight.shape[2:])
                    target_k = list(target_weight.shape[2:])

                    if pt_k != target_k:
                        print(
                            f"[Dynamic Adaptation] Adapting stem kernel for {key}: {pt_k} -> {target_k}"
                        )
                        new_stem_weights[key] = adapt_conv_kernel_size(
                            pt_weight, target_k, pt_k
                        )
            ...
            if need_to_adapt_lpe:
                if lpe_in_stem and "pos_embed" in new_stem_weights:
                    handle_pos_embed_resize(
                        ...
                        downstream_input_patchsize,
                        pt_input_patchsize,
                        new_stem_weights.get(
                            "proj.weight", torch.zeros(1, 1, 1, 1, 1)
                        ).shape[2:],
                    )
        ...
        if not need_to_adapt_lpe and key_to_lpe is not None:
            ...
            if len(lpe_weights) == 1:
                network.get_parameter(key_to_lpe).data = list(lpe_weights.values())[
                    0
                ].to(next(network.parameters()).device)
...
class DynamicPretrainedTrainer_adam_150ep_nomirroring(
    DynamicPretrainedTrainer_adam_150ep
):
    def configure_rotation_dummyDA_mirroring_and_inital_patch_size(self):
        rotation_for_DA, do_dummy_2d_data_aug, initial_patch_size, mirror_axes = (
            super().configure_rotation_dummyDA_mirroring_and_inital_patch_size()
