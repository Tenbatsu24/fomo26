...
def comp_blosc2_params(
    ...
    if len(patch_size) not in (2, 3):
        raise ValueError(
            f"patch_size must be 2D or 3D spatial shape. Got {patch_size}."
        )
...
class nnUNetDatasetBlosc2(nnUNetBaseDataset):
    ...
    def load_case(self, identifier):
        ...
        data = blosc2.open(
            urlpath=data_b2nd_file, mode="r", dparams=dparams, **self.mmap_kwargs
        )

        seg_b2nd_file = join(self.source_folder, identifier + "_seg.b2nd")
        seg = blosc2.open(
            urlpath=seg_b2nd_file, mode="r", dparams=dparams, **self.mmap_kwargs
        )
    ...
    @staticmethod
    def _select_filter(
        arr: np.ndarray, blocks, chunks, codec, clevel
    ) -> "blosc2.Filter":
        """
        ...
        try:
            ...
            slices = tuple(
                slice((s - ss) // 2, (s - ss) // 2 + ss)
                for s, ss in zip(shape, slab_shape)
            )
            slab = np.ascontiguousarray(arr[slices])
            trial_blocks = tuple(
                max(1, min(int(b), ss)) for b, ss in zip(blocks, slab_shape)
            )
            ...
            for f in (blosc2.Filter.NOFILTER, blosc2.Filter.SHUFFLE):
                ...
                comp = blosc2.asarray(
                    slab, chunks=tuple(slab_shape), blocks=trial_blocks, cparams=cparams
                )
    ...
    def save_case(
        ...
        if filters is None:
            data_filters = [
                nnUNetDatasetBlosc2._select_filter(data, blocks, chunks, codec, clevel)
            ]
        else:
        ...
        if filters_seg is None:
            seg_filters = [
                nnUNetDatasetBlosc2._select_filter(
                    seg, blocks_seg, chunks_seg, codec, clevel
                )
            ]
        else:
