...
def adapt_conv_kernel_size(
    ...
    """
    assert len(target_kernel_size) == len(
        pretrained_kernel_size
    ...
    for dim_idx, (target_k, pt_k) in enumerate(
        zip(target_kernel_size, pretrained_kernel_size)
    ):
...
def adapt_encoder_weights_for_architecture(
    ...
    if verbose:
        if pretrained_n_stages > target_n_stages:
            print(
                f"[Dynamic Adaptation] Pretrained encoder has {pretrained_n_stages} stages, "
    ...
    for key, pt_weight in pretrained_encoder_weights.items():
        ...
        if "conv.weight" in rest_of_key and len(pt_weight.shape) >= 4:
            ...
            if pt_kernel != target_kernel:
                ...
                pt_stage_kernel = (
                    pretrained_kernel_sizes[stage_idx]
                    if stage_idx < len(pretrained_kernel_sizes)
                    else pt_kernel
                )
                target_stage_kernel = (
                    target_kernel_sizes[stage_idx]
                    if stage_idx < len(target_kernel_sizes)
                    else target_kernel
                )
                ...
                if verbose:
                    print(
                        f"[Dynamic Adaptation] Adapting kernel for {key}: {pt_kernel} -> {target_kernel}"
                    )

                adapted_weight = adapt_conv_kernel_size(
                    pt_weight, target_kernel, pt_kernel
                )
        ...
        if pt_weight.shape != target_weight.shape:
            adaptation_log[key] = (
                f"skipped (shape mismatch: {pt_weight.shape} vs {target_weight.shape})"
            )
            continue
