...
class NibabelIO(BaseReaderWriter):
    ...
    def write_seg(self, seg: np.ndarray, output_fname: str, properties: dict) -> None:
        # revert transpose
        seg = seg.transpose((2, 1, 0)).astype(
            np.uint8 if np.max(seg) < 255 else np.uint16, copy=False
        )
...
class NibabelIOWithReorient(BaseReaderWriter):
    ...
    def write_seg(self, seg: np.ndarray, output_fname: str, properties: dict) -> None:
        ...
        if not np.allclose(
            ...
            print(
                f"WARNING: Restored affine does not match original affine. File: {output_fname}"
            )
