...
class SimpleITKIO(BaseReaderWriter):
    ...
    def write_seg(self, seg: np.ndarray, output_fname: str, properties: dict) -> None:
        ...
        itk_image = sitk.GetImageFromArray(
            seg.astype(np.uint8 if np.max(seg) < 255 else np.uint16, copy=False)
        )
...
class SimpleITKIOWithReorient(SimpleITKIO):
    def read_images(
        self, image_fnames: Union[List[str], Tuple[str, ...]], orientation="RAS"
    ) -> Tuple[np.ndarray, dict]:
        images = []
        spacings = []
        origins = []
    ...
    def write_seg(self, seg, output_fname, properties):
        ...
        itk_image = sitk.DICOMOrient(
            itk_image, properties["sitk_stuff"]["original_orientation"]
        )
