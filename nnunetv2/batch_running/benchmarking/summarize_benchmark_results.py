...
if __name__ == "__main__":
    ...
    torch_version = (
        "2.1.0.dev20230330"  # "2.0.0"#"2.1.0.dev20230328"  #"1.11.0a0+gitbc2c6ed"  #
    )
        ...
        for p in plans:
            ...
            for c in configs:
                ...
                for d in datasets:
                    ...
                    expected_benchmark_file = join(
                        output_folder, "benchmark_result.json"
                    )
                    ...
                    if isfile(expected_benchmark_file):
                        ...
                        results = [
                            i
                            for i in load_json(expected_benchmark_file).values()
                            if i["num_gpus"] == num_gpus
                            and i["cudnn_version"] == cudnn_version
                            and i["torch_version"] == torch_version
                        ]
                        for r in results:
                    ...
                    for c in configs:
                        ...
                        for g in unique_gpus:
                            if g in all_results[tr][p][c][d].keys():
                                gpu_results.append(
                                    round(
                                        all_results[tr][p][c][d][g]["fastest_epoch"],
                                        ndigits=2,
                                    )
                                )
                            else:
