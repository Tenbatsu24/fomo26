    ...
    if all([i == j for i, j in zip(new_shape, data.shape[1:])]):
        ...
        with torch.no_grad():
            ...
            if is_seg:
                ...
                result = torch.zeros(
                    (data.shape[0], *new_shape), dtype=result_dtype, device=device
                )
                if not memefficient_seg_resampling:
                    ...
                    done_mask = torch.zeros_like(
                        result, dtype=torch.bool, device=device
                    )
                    for i, u in enumerate(unique_values):
                    ...
                    if not torch.all(done_mask):
                        ...
                        result[~done_mask] = unique_values[
                            result_tmp[:, ~done_mask].argmax(0)
                        ].to(result_dtype)
                else:
                    for i, u in enumerate(unique_values):
                        if u == 0:
                ...
                result = F.interpolate(
                    data[None].float(), new_shape, mode=torch_mode, antialias=False
                )[0]
            if input_was_numpy:
...
def resample_torch_fornnunet(
    ...
    if do_separate_z:
        ...
        return resample_torch_simple(
            data, new_shape, is_seg, num_threads, device, memefficient_seg_resampling
        )
...
if __name__ == "__main__":
    ...
    ret_sep = resample_torch_fornnunet(
        use, target_shape, spacing, spacing, is_seg, force_separate_z=False
    )
